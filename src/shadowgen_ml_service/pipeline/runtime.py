...
def build_runtime(settings: Settings) -> PipelineRuntime:
    ...
    real_detector = None
    detector_component = component_status("detector", "mock", grounding.model_name, "mock-v1", True, True, "deterministic fallback detector" if not grounding.available else "real wrapper scaffold exists")
    ...
    real_geometry = None
    geometry_component = component_status("geometry_estimator", "mock", geocalib.model_name, "mock-v1", True, True, "deterministic fallback geometry estimator" if not geocalib.available else "real wrapper scaffold exists")
    if mode != "mock" and geocalib.available:
        try:
            ...
            geometry_component = component_status("geometry_estimator", "real", geocalib.model_name, geocalib.model_version, True, False, "GeoCalib backend active")
        except Exception as exc:
    ...
    real_segmenter = None
    segmenter_component = component_status("segmenter", "mock", birefnet.model_name, "mock-v1", True, True, birefnet.detail or ("deterministic fallback matting stage" if not birefnet.available else "real wrapper scaffold exists"))
    ...
    if mode != "mock" and shadow_pix2pix.available:
        try:
            ...
            shadow_component = component_status("shadow_generator", "real", "V1-GAN|V2-DIFF", "V1-GAN-ready; V2-DIFF-scaffold", True, False, f"active={getattr(shadow, 'model_variant', 'mock')}; V1-GAN ready; V2-DIFF scaffold prepared")
        except Exception as exc:
            shadow_component = component_status("shadow_generator", "mock-fallback", "V1-GAN|V2-DIFF", "stub-v1", True, True, f"pix2pix shadow init failed: {exc}")
